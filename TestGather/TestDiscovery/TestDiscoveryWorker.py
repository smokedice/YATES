...
class TestDiscoveryWorker(object):
    ...
    def __init__(self):
        '''
        @param discoveryCfgName: the name of the filter XML configuration
        '''
        self.test = 0
        self.configurationManager = ConfigurationManager()
        self.config = self.configurationManager.getConfiguration('discovery')
        self.logger = LogManager().getLogger('TestDiscoveryWorker')

        if not hasattr(self.config.configuration, 'PythonNose'):
            self.enabled = False
            return

        self.enabled = self.config.configuration.PythonNose.DiscoveryWorker.enabled == 'true'
        self.testRoot = os.path.abspath(self.config.configuration.PythonNose.SourceLocation.testRoot.PCDATA)
        self.testPath = self.config.configuration.PythonNose.SourceLocation.testPath.PCDATA or ""
        config = ConfigurationManager().getConfiguration('execution').configuration
        self.defaultTestTimeout = int(config.defaultTimeout.PCDATA)
    ...
    def createTests(self):
        ...
        if self.enabled:
            testCases = self.__discoverTests()
        return (testCases, self.testRoot)
    ...
    def __generateTestList(self, testDocs):
        ...
        @return: the list of Test() objects
        '''
        iterations = int(ConfigurationManager().
            getConfiguration("execution").configuration.iterations.PCDATA)
    ...
    def __createTestCase(self, tcase, tfile, testDocs, iterations):
        ...
        return PythonNose(description, iterations, environment,
